...
help = "Use `help(extract_NetCDF)` for more info."
help_con = "Use `help(extract_NetCDF.connection())` for more info."
...
def interactive_con(answer):
    ...
    if answer not in no:
        ...
        logging.info("Usage: `extract_NetCDF.connect(uid, key)`")
...
def check_credentials(uid, key):
    ...
    if not valid_uid:
        return logging.error("Invalid UID. " + f"{help_con}")
    if not valid_key:
        return logging.error("Invalid API Key. " + f"{help_con}")
    return uid, key
